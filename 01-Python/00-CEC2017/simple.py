...
def f6(x, rotation=None, shift=None):
    """
    CEC2017 F6: Shifted Schaffer's F7 Function.

    Notes
    -----
    In the official CEC2017 implementation (cec17_func.cpp), Schaffer's F7
    is computed using ONLY the shifted vector (no effective rotation), and
    then a constant offset of +600 is added. This function reproduces that
    behaviour.

    Args
    ----
    x : array-like, shape (pop_size, D)
        Population of candidate solutions. D must be one of
        {2, 10, 20, 30, 50, 100}.
    rotation : array-like, optional
        Ignored in this implementation; kept for API compatibility.
    shift : array-like, optional
        If None, uses the official CEC2017 shift vector for F6 in dimension D.
    """
    x = np.asarray(x, dtype=float)
    nx = x.shape[1]

    # Official CEC2017 shift vector for F6 (index 5)
    if shift is None:
        shift = transforms.shifts[5][:nx]

    # Shift only (no rotation), matching the CEC2017 C/MEX behaviour
    x_shifted = x - shift

    # Schaffer's F7 + 600 offset
    return basic.schaffers_f7(x_shifted) + 600.0
